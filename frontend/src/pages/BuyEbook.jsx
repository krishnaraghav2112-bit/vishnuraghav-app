...
export default function BuyEbook({ onOpenAuth }) {
  ...
  const buyNow = async () => {
    if (!user) { onOpenAuth("signup"); toast.info("Sign up in 10 seconds to secure your eBook"); return; }
    if (paying) return;
    setPaying(true);
    try {
      ...
      const rzp = new window.Razorpay({
        key: data.razorpay_key, amount: data.amount_paise, currency: "INR",
        name: "Vishnu Raghav", description: product.title || "Mind Health Workbook",
        order_id: data.razorpay_order_id, prefill: data.prefill, theme: { color: "#c9a84c" },
        handler: async (response) => {
          try {
            const { data: v } = await api.post("/assessment/product/verify", {
              order_id: data.order_id, razorpay_payment_id: response.razorpay_payment_id,
          ...
          } catch (e) { toast.error("Payment verification failed. Please contact support."); }
    ...
    } catch (e) { toast.error(formatApiError(e)); setPaying(false); }
  };
  ...
  const downloadEbook = async () => {
    ...
    try {
      ...
      a.href = objUrl; a.download = "Mind-Health-Workbook.pdf";
  ...
  const benefits = [
    { icon: Brain, title: "Silence Overthinking", desc: "Practical exercises to calm the noise in your head and think with clarity." },
    { icon: Clock, title: "Master Your Time", desc: "Simple systems to reclaim your day from distractions and low-value work." },
    { icon: Heart, title: "Emotional Clarity", desc: "Understand and process your emotions instead of being ruled by them." },
    { icon: TrendingUp, title: "Daily Progress", desc: "A 30-day journey with tiny wins that compound into a transformed mindset." },
  ];
  const whatsInside = [
    "30+ pages of practical, no-fluff wisdom",
    "20+ guided exercises and journal prompts",
    "A 30-day mind-reset framework",
    "Hindi + Hinglish — written for the Indian mind",
    "Instant PDF download after payment",
    "Lifetime access — read on phone, laptop, or print it",
    "7-day money-back refund policy",
  ];
  const testimonials = [
    { name: "Priya S.", role: "Delhi · Reader", text: "Finally an author who writes what I feel but couldn't say. This workbook helped me sleep peacefully after months.", stars: 5 },
    { name: "Rahul K.", role: "Mumbai · Student", text: "I completed the 30-day reset in 25 days. My overthinking has reduced by 70%. Worth every rupee.", stars: 5 },
    { name: "Anjali M.", role: "Bengaluru · Coach", text: "I recommend this to all my clients. Vishnu's words go straight to the heart.", stars: 5 },
  ];
  const faqs = [
    { q: "How will I receive the eBook?", a: "Instantly after payment — a download link appears on this page and is also saved in your Dashboard for lifetime access." },
    { q: "Is this a physical book or digital?", a: "Digital PDF only. You can read it on any device or print it if you prefer paper." },
    { q: "What if I don't like it?", a: "We offer a 7-day money-back refund, no questions asked. Just email us." },
    { q: "Is the content in Hindi or English?", a: "Written in warm, simple Hindi + Hinglish — designed for readers who think and feel in both languages." },
    { q: "Can I access it forever?", a: "Yes. One-time payment, lifetime access. Log in anytime from your Dashboard to re-download." },
  ];
        ...
        <div className="max-w-6xl mx-auto px-5 md:px-8 py-12 md:py-20 grid md:grid-cols-2 gap-10 items-center relative">
          <div>
            ...
            {access.has_access && access.pdf_url ? (
              <button onClick={downloadEbook} data-testid="download-ebook-hero" className="w-full sm:w-auto px-8 py-4 rounded-xl bg-green-500 text-white font-bold text-base inline-flex items-center justify-center gap-2 hover:bg-green-600 transition-all">
                <Download className="w-5 h-5" /> Download Your eBook (PDF)
              </button>
            ) : product.is_active && product.has_pdf ? (
              <button onClick={buyNow} disabled={paying} data-testid="buy-ebook-hero" className="w-full sm:w-auto px-8 py-4 rounded-xl bg-gold-gradient text-ink-950 font-black text-base inline-flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-60 shadow-lg shadow-brand-gold/20">
          ...
          <div className="relative">
            <div className="relative mx-auto max-w-sm">
              <div className="aspect-[3/4] rounded-2xl overflow-hidden shadow-2xl shadow-black/50 border border-amber-900/40" style={{ background: "linear-gradient(135deg, #7a4a1e 0%, #5c3416 50%, #3d2410 100%)" }}>
                <div className="h-full w-full flex flex-col items-center justify-between px-6 py-10 text-center relative">
                  <div className="absolute inset-0 opacity-[0.15]" style={{ background: "repeating-linear-gradient(135deg, transparent 0, transparent 8px, rgba(255,255,255,0.08) 8px, rgba(255,255,255,0.08) 9px)" }} />
                  <div className="relative">
                    <div className="text-[10px] tracking-[0.3em] text-amber-100/70 uppercase mb-6">विष्णु राघव प्रस्तुत करते हैं</div>
                    <div className="font-serif text-4xl md:text-5xl font-black text-amber-50 mb-4 leading-tight" style={{ fontFamily: "Georgia, serif" }}>उलझा जीवन</div>
                    <div className="text-sm text-amber-100/90 font-medium mb-6">21 दिनों का माइंड रीसेट सिस्टम</div>
                    <div className="w-16 h-px bg-amber-200/40 mx-auto mb-4" />
                    <div className="text-xs italic text-amber-100/80 leading-relaxed max-w-[220px] mx-auto">मन तब तक शोर करता है,<br/>जब तक उसे सुना नहीं जाता।</div>
                  ...
                  <div className="relative text-center">
                    ...
                    <div className="text-[9px] text-amber-200/50 leading-relaxed max-w-[240px] mx-auto">एक मनोवैज्ञानिक ट्रांसफ़ॉर्मेशन सिस्टम,<br/>प्रेरणादायक किताब नहीं।</div>
                    <div className="text-[10px] text-amber-300/70 font-bold tracking-wider mt-2">authorvishnuraghav.in</div>
                  </div>
                </div>
              </div>
              <div className="absolute -top-3 -right-3 bg-red-500 text-white text-xs font-black px-3 py-1.5 rounded-full shadow-lg rotate-6">{discountPct}% OFF</div>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="space-y-3">
          {faqs.map((f, i) => (
            <div key={i} className="bg-ink-900 border border-white/[0.07] rounded-2xl overflow-hidden">
              <button onClick={() => setOpenFaq(openFaq === i ? null : i)} data-testid={`faq-toggle-${i}`} className="w-full flex items-center justify-between p-5 text-left hover:bg-white/[0.02] transition-colors">
      ...
      <section className="border-t border-white/5">
        <div className="max-w-4xl mx-auto px-5 md:px-8 py-16 text-center">
          ...
          <div className="inline-flex flex-col items-center">
            ...
            {access.has_access && access.pdf_url ? (
              <button onClick={downloadEbook} data-testid="download-ebook-final" className="px-10 py-4 rounded-xl bg-green-500 text-white font-bold text-base inline-flex items-center gap-2 hover:bg-green-600">
                <Download className="w-5 h-5" /> Download Your eBook
              </button>
            ) : product.is_active && product.has_pdf ? (
              <button onClick={buyNow} disabled={paying} data-testid="buy-ebook-final" className="px-10 py-4 rounded-xl bg-gold-gradient text-ink-950 font-black text-base inline-flex items-center gap-2 hover:opacity-90 disabled:opacity-60 shadow-xl shadow-brand-gold/20">
                {paying ? "Processing..." : `📥 Buy Now — ₹${product.price || 199}`} {!paying && <ArrowRight className="w-5 h-5" />}
              </button>
            ) : (
